...
class EventParamFlow(BaseParamsFlow):
    ...
    def end(self):
        ...
        if self.payload_index is None:
            raise Exception("payload index not provided, not possible to assert parameter validity.")

        params_dict = {k: getattr(self, k.replace("-","_")) for k in self.param_defaults.keys()}
